...
build_kernel(){
    ...
    make ${ARGS} || exit 1
    cp out/arch/arm64/boot/Image.gz ${RDIR}/build/Image.gz
    cp out/arch/arm64/boot/Image ${RDIR}/build/Image
}

#build boot.img with AIK (fallback, produces incomplete boot.img)
build_boot() {    
...
#build odin flashable tar with Image.gz + Image + boot.img
build_tar(){
    cd ${RDIR}/build
    tar -cvf "KernelSU-Next-SM-A042F.tar" Image.gz Image boot.img
    echo -e "\n[i] Build Finished..!\n" && cd ${RDIR}
}
